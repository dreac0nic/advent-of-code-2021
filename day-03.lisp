...
(defun calculate-gamma-epsilon (readings)
  ...
  (loop with bits = (make-array (length (car readings)))
        ...
        finally (return (multiple-value-bind (gamma bit-width)
                            (parse-integer (coerce (loop for flag across bits
...
(defun calculate-oxygen-generator-rating (readings)
  ...
  (parse-integer (loop with queue = readings
                       while (cdr queue)
                       for bit from 0 upto (length (car queue))
                       for mask = (if (>= (aref (tilt-reading-values queue) bit) 0)
                                      #\1
                                      #\0)
                       do (setf queue
